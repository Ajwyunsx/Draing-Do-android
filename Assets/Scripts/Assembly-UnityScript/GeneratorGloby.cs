...
[Serializable]
public class GeneratorGloby : MonoBehaviour
{
	...
	public virtual void Awake()
	{
		...
		if (isThunder)
		{
			timer += UnityEngine.Random.Range(-200, 500);
		}
	}

	public virtual void FixedUpdate()
	{
		if (!ON || (isThunder && (Global.DAYRAIN <= 40 || Global.DAYCLOUD < -6f || !(Global.DAYCLOUD < 13f))))
		{
			return;
		}
		if (Dis)
		...
		{
			ownObjects.Add(UnityEngine.Object.Instantiate(ThatObject, Global.CurrentPlayerObject.position + new Vector3(UnityEngine.Random.Range(0f - RndPosition, RndPosition), UnityEngine.Random.Range(0f - RndPosition, RndPosition), ZPosition), Quaternion.identity));
		}
		if (COUNT <= 0)
		{
			return;
